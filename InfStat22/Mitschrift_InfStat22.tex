...
\tsbook{Studierendenmitschrift zur Vorlesung: Einführung in die Inferenzstatistik (SoSe 2022)}
...
\begin{example}[Ein Modell für den radioaktiven Zerfall]
	...
	Damit lässt sich Modell 1 durch Modell 3 approximmieren \\
	...
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
		\draw[<-] (F.south) -- ++ (0,-0.7) node[below] {$ n \to \infty $, \\  $ np=\lambda $ fest }; 
	\end{tikzpicture}
		\vspace{8ex}
...
\begin{proof}[Nachrechnen]
	...
	\begin{aligned}
		...
		& = \frac{np(n-1)p\cdot \ldots \cdot (n-k+1)p}{k!} \left(1-\frac{np}{n}\right)^n  \left(1-\frac{np}{n}\right)^{-k} \\
		& = \frac{np \cdot np \cdot\left(1-\frac{1}{n}\right)\cdot \ldots \cdot np \cdot
		\left(1-\frac{k+1}{n}\right)}{k!} \left(1-\frac{np}{n}\right)^n  \left(1-\frac{np}{n}\right)^{-k} \\
		& \underset{\substack{n \to \infty \\ np=\lambda}}{\longrightarrow} \dfrac{\lambda^k}{k!} e^{-\lambda}1 .
	\end{aligned}
	\]
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
	...
	\begin{remark} 
		Bemerke: \\
		$ np=\lambda $, \\
		$ \left(1-\frac{1}{n}\right)\rightarrow1, $ \\
...
\begin{remark}
	Betrachte einen Schätzer $ \hat{\theta} $ für $ \theta_0 $; das ist eine Größe $ \hat{\theta}=\hat{\theta} \left(X_1,\ldots,X_n \right) $, die alleine aus den Daten $ X_1, \ldots, X_n $ berechenbar ist (oft auch $ \hat{\theta}_n $). \\
...
\begin{definition}[Unverzerrtheit]
	...
	\[
	\forall \theta \in \Theta, \forall i=1,...,p: \tikzmarknode{E}{\E_{\theta_n}}\left(\hat{(\theta)_i}\right) = (\theta)_i.
	\]
	
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
	nodes={inner sep=1pt, align=center, font=\footnotesize}]
	\draw[<-] (E.south) -- ++ (0,-.5) node[below] {Erwartungstreu, wenn $ \theta_n $ \\ der wahre Parameter ist.};
	\end{tikzpicture}
	\vspace{1cm}
...
\begin{remark} Beachte:
	...
	\end{pmatrix} $ ist $ \E_{\theta_n} \left(\hat{\theta}\right) = \begin{pmatrix}
	 \E_\theta \left(\hat{\theta}_1\right) \\ \vdots \\  \E_{\theta_n}  \left(\hat{\theta}_p\right)
\end{pmatrix} $. \\
...
\begin{definition}[Konsistenz]
	...
	\[
	\forall \epsilon > 0, \forall i=1,...,p, \forall \theta \in \Theta : \Prob_{\theta_0} \left(||(\hat{\theta}_n)_i - (\theta)_i || > \epsilon \right) \overset{n\to\infty}{\longrightarrow} 0.
	\]
...
\begin{itemize}
	...
	\item kleine Varianz
	\item Verteilung des $ \hat{\theta_n}-\theta_0 $ bekannt oder approximierbar.
\end{itemize}
...
\begin{satz}
	...
	Ist die Funktion $ f:\R^p \rightarrow \R^p $ stetig in jedem Punkt der Menge $ \left\lbrace \tikzmarknode{M}{\left(m_1(\theta_0),\ldots,m_p(\theta_0)\right)}^\prime = \theta \in \tikzmarknode{T}{\Theta} \right\rbrace (\hexstar) $, dann ist $ \hat{\theta}_{MM} $ konsistent für $ \theta_0 $.
...
\begin{proof} \ \\
	 Zz: Jede Komponente von $ \hat{\theta}_n $ ist konsistent für die entsprechende Komponente von $ \theta_0 $. \\
	
	Sei $j \in \{1,...,p\} $. Laut Voraussetzung ist $ f \colon \R^p \rightarrow \R^p $ stetig im jedem Punkt der Menge $ (\hexstar) $. 
	Dann ist auch $ f(\cdot)_j : \colon \R^p \rightarrow \R^p $ stetig im jedem Punkt von $ (\hexstar) $. \\
	
	Sei $ \theta_0 \in \Theta $ sodass $ m \coloneqq (m_1(\theta_0),..., m_p(\theta_0)) \in (\hexstar) $. 								Mit dem Lemma 1.1 folgt dass \\
	 \[ \forall \epsilon > 0 :  \Prob(| \underbrace{f(\hat{m})_j}_{=(\hat{\theta}_{n})_j} - \underbrace{f(m)_j}_{=(\theta_{0})_j} | > \epsilon ) \rightarrow 0  \]
	 Also ist $(\hat{\theta}_{n})_j $ konsistenf für  $ (\theta_{0})_j $.
 	
\end{proof}

\begin{satz}
	Betrachte ein parametrisches Modell wo X Verteilung abh. von $\theta \leq \Theta \in \R$. \\
	Seien $X_1, X_2$ i.i.d. Kopien von X falls  $  \theta_0 = f (m_1(\theta_0) ) \ ( $ für $ \ \theta \in \Theta) $ für eine Funktion f, die auf der Menge $ \{m_1(\theta): \theta \in \Theta \}$ stetig ist. Dann ist der MM-Schätzer  \[ \hat{\theta}_n=f(\frac{1}{n} \sum_{i=1}^{n} X_i ) \]  kosistent für $\theta_0$.
...
\begin{example}[Poisson-Modell]
	...
	Für $ Z_1,\ldots, Z_n $ i.i.d. mit $ \E(Z_1) = \mu_0 $ und $ \Var(Z_1) = \sigma^2 $. Ist ein KI für $ \mu $ mit \textbf{nominaler} \index{Konfidenzinterval!nominal} Überdeckungswahrscheinlichkeit $ 1 - \alpha $ 
	\[
...
\begin{proof}
	...
	Dann ist $ R(\cdot) $ stetig im Punkt $ \mu_Z $: 
	\[
	...
	Ersetze Z durch $\bar{Z}_n$
	
	\[
		f^\prime(\mu_Z)  = \frac{f(\bar{Z}_n)-f(\mu_Z)}{\bar{Z}_n-\mu_Z} + R(\bar{Z}_n) \\
	\]
...
Die Maximum-Likelihood Methode \index{Schätzer!Maximum-Likelihood Methode} maximiert $ L(\theta) $ bzw. $ l(\theta) $ über $ \theta\in\Theta $: \\
\[
...
\begin{definition}
	Betrachte ein Parametrisches Modell mit Parameter $ \theta_0 \in \Theta \leq \R $. Für einen Schätzer $ \hat{\theta}_n $ ist der \textbf{mittlere quadratische Fehler (MSE)} \index{MSE} definiert als 
	\[
...
Gegeben einen Schätzer $ \hat{\theta}_n = \hat{\theta}_n(X_1, \ldots, X_n) \in \R^k $ für $ \theta_0 $ betrachtet man den \textbf{mittleren quadratischen Fehler} \index{MSE} 
	\[
...
\begin{definition}
	...
	\begin{itemize}
		\item $\hat{\theta}$ heißt \textbf{unzulässig} \index{Schätzer!Unzulässigkeit} (in der Klasse $\kappa$), wenn es einen Schätzer $\tilde{\theta}$ aus $\kappa$ gibt sodass:
...
\begin{definition}
	...
	\] ist die \underline{Fisher-Information} \index{Fisher-Information} (von $ X $ über $ \theta_0 $), sofern diese wohldefiniert ist.
\end{definition}
...
\begin{example}[Normalverteilung - Beide unbekannt]
	$ X_1,\ldots,X_n $ i.i.d. $ N(\mu_0,\sigma_0^2), \; \mu_0 \in \R, \; \sigma_0^2 > 0 $.
...
\begin{remark}
	...
	\[
	\forall \mu_0 \in \R^p \colon MSE(\tikzmarknode{JS}{\hat{\mu}_{JS}},\mu_0) < MSE(\tikzmarknode{X}{X},\mu_0)
	\]
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
	...
	\[
	MSE({\hat{\mu}_{JS}}, 0 ) =2, \  MSE(X,0)=p.
	\]
	 
	
	Dieser Schätzer ist gegeben durch 
	\[
	\hat{\mu}_{JS} = \underbrace{\left(1-\frac{p-2}{X^{\prime} X}\right)}_{\text{Kontraktionsfaktor <1 }} \tikzmarknode{ML}{X}.
	\]
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
...
\end{remark}
	...
	\begin{remark}
		...
		In der Praxis wird der James-Stein-Schätzer fast nie verwendet. Aber als so genannter Shrinkage-Schätzer lieferte $ \hat{\mu}_{JS} $ die Inspiration für zahlreiche moderne Methoden wie LASSO, SVM, LARS, Dantzig-Selektor, $ \ldots $
	\end{remark}

\textbf{Zur Motivation des James-Stein-Schätzers:}

	$ X\sim N(\mu,I_p), \; \hat{\mu}_{JS} = \left(1-\frac{p-2}{X^\prime X}\right)X , \ (p \ge 3)$. 
	
	Betrachte eine Approximation, wo $ p\to\infty $ geht, wobei
	\[
	\underbrace {\frac{\tikzmarknode{S}{||\mu||^2}}{\tikzmarknode{N}{p}}}_{singal-to-noice \ ratio} \xrightarrow{p\to\infty} \rho^2 > 0 \in [0,1].
	\]
	...
	\begin{remark}
		Beachte: Als Schätzer für $ \mu $ ist $ X $ (im Mittel) ''zu lang'': 
		\[
		\begin{aligned}
			 \text{Länge von } \mu: ||\mu||^2 \\
			 \text{Länge von } X: \E(||X||^2) & = \E(X^\prime X) \\
...
\begin{proof}[Nachrechnen]
	...
	\[
	\begin{aligned}
		\Prob(S\ge \tilde{c}\; |H_0) = \tilde{\alpha} & \ge a = \Prob(S\ge s\; |H_0) \\
		\Leftrightarrow \tilde{c} & \le s \\
		\equiv \tikzmarknode{S}{s} & \ge \tikzmarknode{C}{\tilde{c}} \\
		\\
...
\end{proof}
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
		\draw[<-] (S.south) -- ++ (-3.5,-.5) node[below] {Beobachtbarer Wert der Test-Statistik};
		\draw[<-] (C.south) -- ++ (2.5,-.5) node[below] {Kritische Wert des Tests};
	\end{tikzpicture}
\begin{remark}
	...
	\]
		...
		\begin{tikzpicture}
			\begin{axis}[no markers, domain=0:10, samples=100,
				...
				\node[coordinate, pin={$1-\Phi(c)$}] at (axis cs: 2.5, 0.01){};
				\node[coordinate, pin={$\Phi(-c)$}] at (axis cs: -2.5, 0.01){};
			\end{axis}
		\end{tikzpicture}
	\end{center}
...
Allgemeine Modell: Hier ist die Likelihood-Ratio-Statistik (L-R-Statistik) \index{Likelihood-Ratio} ist definiert als 
\[
...
\begin{example}[z-Test mit linksseitiger Alternative]
...
Im folgenden \textbf{Neyman-Pearson Lemma}\index{Neyman-Pearson Lemma} betrachtet man sogenannte \textbf{simple Hypothesen}, unter denen die Verteilung der Daten jeweils komplett spezifiziert ist:
...
(Ohne Beweis)
	...
	\begin{example}[Kategoriale Daten, einfache Nullhypothese] 
...
Ist $\textbf{X}_{(n\times1)}$ ein Zufallsvektor, dann ist die so genannte
Varianz/Kovarianz-Matrix von $\textbf{X}$ die $n\times n$ Matrix
\[
\VC(\textbf{X})=\left(\VC(X)_{ij}\right)_{i=1\qquad j=1}^{n\qquad\;\;\; n},
\]
wobei 
\[
\VC(\textbf{X})=\begin{cases}
\Var(X_{i}) & :i=j\\
\Cov(X_{i},X_{j}) & :i\ne j
\end{cases}
\]
...
\begin{remark}
	Bemerkung: Weil $ \Cov\left(X_i, X_j) \right)= \Cov\left(X_j,X_i \right) $ ist, ist $ \VC(\textbf{X}) $ immer symmetrisch:  \[ (\VC(\textbf{X}))^\prime = \VC(\textbf{X}) \]
\end{remark}
...
Sind $ \lambda_i $ die Eigenwerte und $ \textbf{u}_i $ die Eigenvektoren von $ \textbf{A}_{(n \times n)} $, dann gilt 
	\[
	\begin{aligned}
		&\textbf{A} \textbf{u}_i & = & \lambda_i \textbf{u}_i , \quad 1\le i \le n. &&\\
		&& \equiv & \textbf{A}\underbrace{(\textbf{u}_1,\ldots,\textbf{u}_n)}_{\textbf{U}} & = & (\lambda_1 \textbf{u}_1, \ldots, \lambda_n \textbf{u}_n) \\
		&& \equiv & \textbf{A} \textbf{U} & = & \textbf{U}\boldsymbol{\Omega}\\
		&&\Rightarrow & \textbf{A} \underbrace{\textbf{U}\textbf{U}^\prime}_{\textbf{I}_n} & = & \textbf{U}\boldsymbol{\Omega} \textbf{U}^\prime 
	\end{aligned}
	\] 
	Also: $ \textbf{A} = \textbf{U} \boldsymbol{\Omega} \textbf{U}^\prime $. 
	\[
	\textbf{A}x = \tikzmarknode{U}{\textbf{U}} \tikzmarknode{O}{\boldsymbol{\Omega}} \tikzmarknode{UP}{\textbf{U}^\prime} x
	\]
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
	...
	\[ \textbf{A}^2 = \textbf{A}\textbf{A} = \textbf{U} \boldsymbol{\Omega} \textbf{U}^\prime \textbf{U} \boldsymbol{\Omega} \textbf{U}^\prime = \textbf{U} \boldsymbol{\Omega}^2 \textbf{U}^\prime . \] 
	
	\[ \textbf{A}^{\frac{1}{2}} \coloneqq \textbf{U} \tikzmarknode{D}{\boldsymbol{\Omega}^{\frac{1}{2}}} \textbf{U}^\prime . \]
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
		\draw[<-] (D.south) -- ++ (0,-.5) node[below] {diag$ \left( \sqrt{\lambda_1}, \ldots, \sqrt{\lambda_n} \right) $};
	\end{tikzpicture}
	\vspace{4ex}

	\[ \textbf{A}^{\frac{1}{2}} \textbf{A}^{\frac{1}{2}} = \textbf{U} \boldsymbol{\Omega}^{\frac{1}{2}} \textbf{U}^\prime \textbf{U} \boldsymbol{\Omega}^{\frac{1}{2}} \textbf{U}^\prime = \textbf{U} \boldsymbol{\Omega}^{\frac{1}{2}} \boldsymbol{\Omega}^{\frac{1}{2}} \textbf{U}^\prime = \textbf{A}. \]
	
	Ist $ \textbf{A} $ invertierbar, dann sind alle $ \lambda_i \ne 0 $, sodass $ \boldsymbol{\Omega}^{-1} = \text{diag} \left(\frac{1}{\lambda_1}, \ldots, \frac{1}{\lambda_n} \right)  $ wohldefiniert ist.\\
	Dann ist aber $ \textbf{A}^{-1} = \textbf{U} \boldsymbol{\Omega}^{-1} \textbf{U}^\prime $, denn 
	\[
	\begin{aligned}
		\textbf{A} \textbf{U} \boldsymbol{\Omega}^{-1} \textbf{U} & = \textbf{U} \boldsymbol{\Omega} \underbrace{\textbf{U}^\prime \textbf{U}}_{\textbf{I}_n} \boldsymbol{\Omega}^{-1} \textbf{U}^\prime & = \textbf{U} \textbf{U}^\prime & = \textbf{I}_n. \\
		\textbf{U} \boldsymbol{\Omega}^{-1} \textbf{U}^\prime \textbf{A} & = \ldots & & = \textbf{I}_n.
	\end{aligned}
	\]
	
	Weiters ist dann 
	\[
	\textbf{A}^{-\frac{1}{2}} = \textbf{U} \tikzmarknode{D2}{\boldsymbol{\Omega}^{-\frac{1}{2}}} \textbf{U}^\prime
	\] 
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
		\draw[<-] (D2.south) -- ++ (0,-.5) node[below] {diag$ \left( \frac{1}{\sqrt{\lambda_1}}, \ldots, \frac{1}{\sqrt{\lambda_n}} \right) $};
	\end{tikzpicture}
	\vspace{4ex}
	
	ebenfalls wohldefiniert und erfüllt die Beziehung 
	\[
	\textbf{A}^{-\frac{1}{2}} \textbf{A} \textbf{A}^{-\frac{1}{2}} = \textbf{I}_n \quad \text{(Details: Übung.)}
	\]
	
	\begin{remark}
		Bemerkung: Ist $ \textbf{X} $ ein $ n $-dimensionaler Zufallsvektor mit Mittelwert $ 0 $ und VC-Matrix $ \boldsymbol{\Sigma} > 0 $, dann gilt für den Zufallsvektor $ \textbf{Y} = \boldsymbol{\Sigma}^{-\frac{1}{2}} \textbf{X} $ ($ n $-dim.): 
		\[ \begin{aligned}
		\E(\textbf{Y}) & = \boldsymbol{0} \\
		\VC(\textbf{Y}) & = \textbf{I}_n.
		\end{aligned}
		\]
	\end{remark}
...
\begin{proof}
	...
	Da dies für beliebige $ \mathbf{X}, \mathbf{Y} \in \R^n  $ gilt, folgt $ \mathbf{P} = \mathbf{P}^\prime $. \\
	
	Weiters ist $ \mathbf{X}^\prime \mathbf{P}\mathbf{Y} = \mathbf{X}^\prime \mathbf{P}^\prime \mathbf{P}\mathbf{Y} $ \\
	
	$ \Rightarrow \mathbf{P} = \mathbf{P}^\prime \mathbf{P} \tikzmarknode{P}{=} \mathbf{P}\mathbf{P} = \mathbf{P}^2 $.
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
		\draw[<-] (P.south) -- ++ (0,-.5) node[below] {$ \mathbf{P}^\prime = \mathbf{P} $};
	\end{tikzpicture}
	\vspace{4ex}
...
\begin{remark}
	Es gilt auch die Umkehrung:
		Ist $ \mathbf{P}_{(n \times n)} $ mit $ \mathbf{P}^2=\mathbf{P} $  und $ \mathbf{P}=\mathbf{P}^\prime $, dann ist $ \mathbf{P} $ die Matrix einer Orthogonalprojektion (ohne Beweis).
	\end{remark}
...
\begin{corollary}
	...
	Ist $ \mathbf{P}_{(n\times n)} $ die Matrix einer Orthogonalprojektion auf einen $ k $-dimensionalen Teilraum $ (0 \le k \le n) $, dann sind genau $ k $ Eigenwerte von $ \mathbf{P} $ gleich 1 und der Rest ist 0.
...
\begin{example}
	...
	(Beachte: $ \hat{\sigma}_n^2 = \frac{1}{n-1} ||\mathbf{X}-\boldsymbol{\iota}\bar{X}||^2 $).\\
	Wissen: $ \mathbf{X}-\boldsymbol{\iota} \bar{X} \sim N\left(0, \sigma^2\left(\mathbf{I}_n -\boldsymbol{\iota}\frac{1}{n}\boldsymbol{\iota}^\prime \right)\right) $. \\
	Für $ \mathbf{Z} \sim N(0,\mathbf{I}_n) $ ist 
	\[
	\begin{aligned}
		\sigma \underbrace{\left(\mathbf{I}_n -\boldsymbol{\iota}\frac{1}{n}\boldsymbol{\iota}^\prime \right)}_{P} \mathbf{Z} & \sim N(0, \underbrace{\sigma \mathbf{P}\mathbf{I}_n \mathbf{P}^\prime \sigma}_{\substack{= \sigma^2\mathbf{P}\mathbf{P}^\prime \\ = \sigma^2 \mathbf{P}\mathbf{P} \\ = \sigma^2 \mathbf{P}}}) \\
		& = N(0, \sigma^2 \mathbf{P}) \\
		& \sim \mathbf{X}- \boldsymbol{\iota} \bar{X}.
	\end{aligned}
	\]
	
	\[
	\begin{aligned}
		||\mathbf{X}-\boldsymbol{\iota}\bar{X}||^2 & \sim ||\sigma \underbrace{\left(\mathbf{I}_n -\boldsymbol{\iota}\frac{1}{n}\boldsymbol{\iota}^\prime \right)}_{P} \mathbf{Z} ||^2 \\
		& = (\sigma \mathbf{P} \mathbf{Z})^\prime(\sigma \mathbf{P} \mathbf{Z}) \\
		& = \sigma^2 \mathbf{Z}^\prime \mathbf{P}^\prime \mathbf{P}\mathbf{Z} \\
		& = \sigma^2 \mathbf{Z}^\prime \mathbf{P}\mathbf{Z}\\
